...
class GoodController extends Controller
{
    ...
    //    Log::channel('stderr')->info($request);
       $validator = Validator::make($request->all(), [
        ...
        try{
            if ($validator->fails())
            {Log::channel('stderr')->info(99999, $request->all());
                Toastr::success(trans('messages.failed_create_product'));
            }else{
                ...
                Log::channel('stderr')->info("000000");
                ...
                Log::channel('stderr')->info(111111);
                //combinations end
                ...
                Log::channel('stderr')->info(22222);
                $attributes = [];
                if($request->attribute_id != null){
                    array_push($attributes, $request->attribute_id);
                }
                $product->attributes = $request->has('attribute_id') ? json_encode($attributes) : json_encode([]);
                ...
                Log::channel('stderr')->info($product);
                $product->save();
                Log::channel('stderr')->info(44444);
    
                $data = [];
                foreach ($request->lang as $index => $key) {
                    if ($request->name[$index] && $key != 'en') {
                        array_push($data, array(
                            'translationable_type' => 'App\Models\Good',
    ...
    {
        $product = Good::withoutGlobalScope(RestaurantScope::class)->where(['id' => $id])->first();
    ...
    public function change_feature(Good $product, Request $request)
    {   
        Log::channel('stderr')->info(121212);
        Log::channel('stderr')->info($product);
        $product->f_featured = $request->f_featured;
        $product->updated_at = now();
        $product->save();
        return back();
    }
    
    public function change_trending(Good $product, Request $request){
        $product->f_trending = $request->f_trending;
        $product->updated_at = now();
        $product->save();
        return back();
    }
    ...
    {
        $product = Good::withoutGlobalScope(RestaurantScope::class)->withoutGlobalScope('translate')->findOrFail($id);
        if(!$product)
        {
        ...
        return view('admin-views.product.edit', compact('product', 'category_ids', 'categories', 'addonArr'));
    }

    public function status(Request $request)
    {
        $product = Good::withoutGlobalScope(RestaurantScope::class)->findOrFail($request->id);
        $product->status = $request->status;
        $product->save();
        Toastr::success(trans('messages.product_status_updated'));
        return back();
    }
    ...
    public function update(Request $request, $id)
    {
        try{
            ...
            if ($validator->fails())
            {
                ...
                $p = Good::withoutGlobalScope(RestaurantScope::class)->find($id);
    ...
    {
        $product = Good::withoutGlobalScope(RestaurantScope::class)->withoutGlobalScope('translate')->find($request->id);
    ...
    public function variant_price(Request $request)
    {
        ...
        {
            $product = Good::withoutGlobalScope(RestaurantScope::class)->find($request->id);
        }
        else
        {
    ...
    {
        $products = Good::withoutGlobalScope(RestaurantScope::class)->with('restaurant')->whereHas('restaurant', function($query)use($request){
            $query->where('zone_id', $request->zone_id);
        })->get();
        $res = '';
        if(count($products)>0 && !$request->data)
        {
            $res = '<option value="' . 0 . '" disabled selected>---Select---</option>';
        }
    ...
    public function list(Request $request)
    {
        ...
        $products = Good::withoutGlobalScope(RestaurantScope::class)
        ...
        // return view('admin-views.product.list', compact('products','restaurant','category'));
        return view('admin-views.product.list', compact('products','restaurant'));
    }

    public function search(Request $request){
        $key = explode(' ', $request['search']);
        $products=Good::withoutGlobalScope(RestaurantScope::class)->where(function ($q) use ($key) {
